...
while current < final:
    ...
    for gameD in GAMES.values():
        if gameD["timezone"] not in (TIMEZONES["JST"],):
            continue
        print(current)
        sleep(2)
        initial_msg = (
        ...
        pings_unpack = list(zip_longest(*pings))
        ...
        for artist in artists:
            ...
            artist_ping_list = artist_pings[gameD["pingColumns"].index("users")].split(
                ","
            )
            ...
            for bonus in bonuses:
                start = datetime.strptime(
                    bonus[gameD["bonusColumns"].index("bonus_start")].replace("\r", ""),
                    "%Y-%m-%d",
                )
                start = start.replace(tzinfo=ZoneInfo("Asia/Manila"))
                end = datetime.strptime(
                    bonus[gameD["bonusColumns"].index("bonus_end")].replace("\r", ""),
                    "%Y-%m-%d",
                )
            ...
            if birthday_bonuses:
                ...
                assert birthday_bonus_end == next_birthday_end
                ...
                print(last_birthday_start)
                print(birthday_bonus_start)
                ...
                for bonus in birthday_bonuses:
                    ...
                    start = start.replace(tzinfo=ZoneInfo("Asia/Manila"))
                    if start == current:
                        assert start == birthday_bonus_start
                        msg = (
